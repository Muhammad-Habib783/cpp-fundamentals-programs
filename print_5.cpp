...
int main() {
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            // Logic to print the pattern of '5'
            if ((i == 0 && j > 0 && j < cols - 1) || // Top horizontal line
                ...
                (j == 0 && i != 0 && i != rows / 2 && i != rows - 1) || // Left vertical line (except for top, middle, and bottom)
                (j == cols - 1 && i >= rows / 2 && i != rows - 1)) { // Right vertical line (for bottom half)
                cout << "*";
            } else {
                cout << " ";
            }
